...
public class Sync {
    ...
    private static boolean compareFiles(String path1, String path2) throws IOException {
        byte[] allBytes1 = Files.readAllBytes(Paths.get(path1)), allBytes2 = Files.readAllBytes(Paths.get(path2));
        if (allBytes1.length != allBytes2.length) {
            return false;
        } else {
            for (int i = 0; i < allBytes1.length; i++) {
                if (allBytes1[i] != allBytes2[i]) {
                    return false;
                }
            }
            return true;
